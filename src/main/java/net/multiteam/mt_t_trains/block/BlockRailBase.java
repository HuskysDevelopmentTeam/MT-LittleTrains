...
public abstract class BlockRailBase extends BlockMod {
    ...
    public BlockRailBase(boolean isPowered, String name) {
    ...
    public AxisAlignedBB getBoundingBox(IBlockState state, IBlockAccess source, BlockPos pos) {
        BlockRailBase.EnumRailDirection BlockRail$enumraildirection = state.getBlock() == this ? getRailDirection(source, pos, state, null) : null;
        return BlockRail$enumraildirection != null && BlockRail$enumraildirection.isAscending() ? ASCENDING_AABB : FLAT_AABB;
    }
    ...
    public void neighborChanged(IBlockState state, World worldIn, BlockPos pos, Block blockIn, BlockPos fromPos) {
        if (!worldIn.isRemote) {
            BlockRailBase.EnumRailDirection BlockRail$enumraildirection = getRailDirection(worldIn, pos, worldIn.getBlockState(pos), null);
            boolean flag = false;
            ...
            if (BlockRail$enumraildirection == BlockRailBase.EnumRailDirection.ASCENDING_EAST && !worldIn.getBlockState(pos.east()).isSideSolid(worldIn, pos.east(), EnumFacing.UP)) {
                flag = true;
            } else if (BlockRail$enumraildirection == BlockRailBase.EnumRailDirection.ASCENDING_WEST && !worldIn.getBlockState(pos.west()).isSideSolid(worldIn, pos.west(), EnumFacing.UP)) {
                flag = true;
            } else if (BlockRail$enumraildirection == BlockRailBase.EnumRailDirection.ASCENDING_NORTH && !worldIn.getBlockState(pos.north()).isSideSolid(worldIn, pos.north(), EnumFacing.UP)) {
                flag = true;
            } else if (BlockRail$enumraildirection == BlockRailBase.EnumRailDirection.ASCENDING_SOUTH && !worldIn.getBlockState(pos.south()).isSideSolid(worldIn, pos.south(), EnumFacing.UP)) {
                flag = true;
            }
    ...
    public static enum EnumRailDirection implements IStringSerializable {
        ...
        static {
            for (BlockRailBase.EnumRailDirection BlockRail$enumraildirection : values()) {
                META_LOOKUP[BlockRail$enumraildirection.getMetadata()] = BlockRail$enumraildirection;
            }
        }
    ...
    public class Rail {
        ...
        public Rail(World worldIn, BlockPos pos, IBlockState state) {
            ...
            BlockRailBase.EnumRailDirection BlockRail$enumraildirection = block.getRailDirection(worldIn, pos, state, null);
            this.isPowered = !this.block.isFlexibleRail(worldIn, pos);
            this.canMakeSlopes = this.block.canMakeSlopes(worldIn, pos);
            this.updateConnectedRails(BlockRail$enumraildirection);
        }
        ...
        private void removeSoftConnections() {
            for (int i = 0; i < this.connectedRails.size(); ++i) {
                BlockRailBase.Rail BlockRail$rail = this.findRailAt(this.connectedRails.get(i));

                if (BlockRail$rail != null && BlockRail$rail.isConnectedToRail(this)) {
                    this.connectedRails.set(i, BlockRail$rail.pos);
                } else {
        ...
        private void connectTo(BlockRailBase.Rail rail) {
            ...
            BlockRailBase.EnumRailDirection BlockRail$enumraildirection = null;

            if (flag || flag1) {
                BlockRail$enumraildirection = BlockRailBase.EnumRailDirection.NORTH_SOUTH;
            }

            if (flag2 || flag3) {
                BlockRail$enumraildirection = BlockRailBase.EnumRailDirection.EAST_WEST;
            }

            if (!this.isPowered) {
                if (flag1 && flag3 && !flag && !flag2) {
                    BlockRail$enumraildirection = BlockRailBase.EnumRailDirection.SOUTH_EAST;
                }

                if (flag1 && flag2 && !flag && !flag3) {
                    BlockRail$enumraildirection = BlockRailBase.EnumRailDirection.SOUTH_WEST;
                }

                if (flag && flag2 && !flag1 && !flag3) {
                    BlockRail$enumraildirection = BlockRailBase.EnumRailDirection.NORTH_WEST;
                }

                if (flag && flag3 && !flag1 && !flag2) {
                    BlockRail$enumraildirection = BlockRailBase.EnumRailDirection.NORTH_EAST;
                }
            }

            if (BlockRail$enumraildirection == BlockRailBase.EnumRailDirection.NORTH_SOUTH && canMakeSlopes) {
                if (BlockRailBase.isRailBlock(this.world, blockpos.up())) {
                    BlockRail$enumraildirection = BlockRailBase.EnumRailDirection.ASCENDING_NORTH;
                }

                if (BlockRailBase.isRailBlock(this.world, blockpos1.up())) {
                    BlockRail$enumraildirection = BlockRailBase.EnumRailDirection.ASCENDING_SOUTH;
                }
            }

            if (BlockRail$enumraildirection == BlockRailBase.EnumRailDirection.EAST_WEST && canMakeSlopes) {
                if (BlockRailBase.isRailBlock(this.world, blockpos3.up())) {
                    BlockRail$enumraildirection = BlockRailBase.EnumRailDirection.ASCENDING_EAST;
                }

                if (BlockRailBase.isRailBlock(this.world, blockpos2.up())) {
                    BlockRail$enumraildirection = BlockRailBase.EnumRailDirection.ASCENDING_WEST;
                }
            }

            if (BlockRail$enumraildirection == null) {
                BlockRail$enumraildirection = BlockRailBase.EnumRailDirection.NORTH_SOUTH;
            }

            this.state = this.state.withProperty(this.block.getShapeProperty(), BlockRail$enumraildirection);
            this.world.setBlockState(this.pos, this.state, 3);
        }

        private boolean hasNeighborRail(BlockPos posIn) {
            BlockRailBase.Rail BlockRail$rail = this.findRailAt(posIn);

            if (BlockRail$rail == null) {
                return false;
            } else {
                BlockRail$rail.removeSoftConnections();
                return BlockRail$rail.canConnectTo(this);
            }
        }
            ...
            if (initialPlacement || this.world.getBlockState(this.pos) != this.state) {
                ...
                for (int i = 0; i < this.connectedRails.size(); ++i) {
                    ...
                    if (BlockRail$rail != null) {
                        ...
                        if (BlockRail$rail.canConnectTo(this)) {
                            BlockRail$rail.connectTo(this);
                        }
                    }
                }
            }
